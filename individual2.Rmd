...
countaccgroup %>% group_by("County_Name") %>% ggplot(aes(x = long, y = lat,fill=(total_crashes))) + geom_polygon(aes(group=group))
```

Test to see if values are being summed in map instead of taking the true value of total_crashes
```{r}
washtest = countaccgroup %>% filter(region=="washington") %>% group_by(County_Name) %>% summarise(total_crashes=n())
washtest = washtest %>% left_join(counties,by="County_Name")
max(washtest$total_crashes)
washtest %>% ggplot(aes(x = long, y = lat,fill=(total_crashes))) + geom_polygon(aes(group=group))
```

This method works because the largest value is 98 and the scale is from 0 to around 100